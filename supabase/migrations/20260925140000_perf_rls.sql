-- ============================================================================
-- perf(rls): auth.uid() -> (select auth.uid()) trong policy RLS + hàm helper
-- Pha 2 tối ưu tốc độ 9/2026 (Agent E). Sinh tự động từ pg_policies /
-- pg_get_functiondef của project (đọc 25/9/2026), KHÔNG viết tay.
-- ============================================================================
-- Vì sao: auth.uid() là hàm STABLE; viết trần trong policy thì planner gọi lại
-- cho TỪNG DÒNG được quét (SubPlan). Bọc thành (select auth.uid()) → InitPlan,
-- tính đúng 1 lần / câu lệnh. Với hàm helper `language sql stable security definer`
-- (is_admin, teaches_student…) planner inline thân hàm vào policy nên cũng phải
-- sửa bên trong thân. Bằng chứng cần sửa: profiles.idx_scan = 2.8 triệu
-- (perf/DB_REPORT.md §3.1) — is_admin() đang chạy lặp theo dòng.
--
-- ĐIỀU KIỆN KHÔNG ĐỔI: chỉ đổi cú pháp; bỏ "(select …)" đi thì ra lại đúng chuỗi
-- pg_policies.qual / with_check và pg_get_functiondef cũ (script sinh có assert).
-- Không nới, không siết quyền. Tên policy, roles, cmd, permissive giữ nguyên.
-- 2 policy tautology (attendance_sessions / equipment_breakdown_reports:
-- `e.course_id = e.course_id`) được chép lại y nguyên — chờ thầy quyết, xem
-- perf/RLS_REPORT.md.
--
-- Cách chạy: Dashboard → SQL Editor → dán cả file, Run (1 transaction; nếu một
-- lệnh lỗi thì rollback toàn bộ, không có trạng thái nửa chừng).
--   hoặc: supabase db query --linked -f supabase/migrations/20260925140000_perf_rls.sql
-- KHÔNG dùng `supabase db push`. Rollback: perf/rollback/20260925140000_perf_rls.down.sql
-- (định nghĩa cũ, sinh cùng lúc từ DB) — hoặc chạy lại docs/supabase-migration-*.sql gốc.
-- ============================================================================

begin;

-- ---------------------------------------------------------------------------
-- A. Hàm helper (create or replace, giữ nguyên chữ ký / volatility / search_path)
-- ---------------------------------------------------------------------------
-- assists_class: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select exists (
    ...
    where tac.class_id = p_class and a.user_id = (select auth.uid()) and a.active
  );
$function$;

-- can_manage_class: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select public.is_admin() or exists (
    ...
    where class_id = p_class_id and instructor_id = (select auth.uid())
  );
$function$;

-- can_manage_course: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select public.is_admin() or exists (
    ...
    where course_id = p_course_id and instructor_id = (select auth.uid())
  );
$function$;

-- get_periodic_rank: 1 chỗ auth.uid() trần trong thân hàm
CREATE OR REPLACE FUNCTION public.get_periodic_rank(p_class_id bigint)
 ...
 SET search_path TO 'public'
AS $function$
  select * from public.get_periodic_rank_of((select auth.uid()), p_class_id);
$function$;

-- is_admin: 1 chỗ auth.uid() trần trong thân hàm
...
-- is_course_member: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select exists (
    ...
    where course_id = p_course_id and student_id = (select auth.uid()) and status = 'active'
  );
$function$;

-- is_homeroom_classmate: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select exists (
    ...
    where e.student_id = p_profile_id and m.student_id = (select auth.uid())
  );
$function$;

-- is_homeroom_monitor: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select exists (
    ...
    where course_id = p_course_id and student_id = (select auth.uid())
  );
$function$;

-- is_parent_of: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select exists (
    ...
    where parent_id = (select auth.uid()) and student_id = p_student and claimed_at is not null
  );
$function$;

-- manages_class: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select public.is_admin() or exists (
    ...
    where ci.class_id = p_class and ci.instructor_id = (select auth.uid())
  );
$function$;

-- rank_is_staff: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select public.is_admin() or exists (
    ...
    where p.id = (select auth.uid()) and p.role = 'instructor' and p.admin_area = 'thpt'
  );
$function$;

-- rank_my_status: 1 chỗ auth.uid() trần trong thân hàm
CREATE OR REPLACE FUNCTION public.rank_my_status()
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ select public.rank_status_of((select auth.uid())); $function$;

-- rank_my_titles: 1 chỗ auth.uid() trần trong thân hàm
CREATE OR REPLACE FUNCTION public.rank_my_titles()
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ select public.rank_titles_of((select auth.uid())); $function$;

-- student_outcome_gaps: 1 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  with students as (
    ...
    where s.id = (select auth.uid()) or public.teaches_student(s.id) or public.is_parent_of(s.id)
  ),
  recent as (
...
-- ta_current_assistant_id: 1 chỗ auth.uid() trần trong thân hàm
CREATE OR REPLACE FUNCTION public.ta_current_assistant_id()
 ...
 SET search_path TO 'public'
AS $function$
  select id from public.ta_assistants where user_id = (select auth.uid());
$function$;

-- teaches_student: 2 chỗ auth.uid() trần trong thân hàm
...
AS $function$
  select public.is_admin() or exists (
    ...
    join public.ta_assistants a on a.id = tac.assistant_id
    where uc.user_id = p_student and a.user_id = (select auth.uid()) and a.active
  );
$function$;

-- thpt_has_registration: 2 chỗ auth.uid() trần trong thân hàm
  ...
  select auth.uid() is not null and exists (
    ...
    where r.course_id = p_course_id
      and (r.student_id = (select auth.uid())
           or r.registered_by = (select auth.uid())
           or (r.student_id is not null and public.is_parent_of(r.student_id)))
  );
$function$;

-- unread_notification_count: 1 chỗ auth.uid() trần trong thân hàm
CREATE OR REPLACE FUNCTION public.unread_notification_count()
 ...
 SET search_path TO 'public'
AS $function$
  select count(*)::int from public.notifications where user_id = (select auth.uid()) and read_at is null;
$function$;

-- ---------------------------------------------------------------------------
-- B. Policy: drop + create lại cùng tên, cùng roles/cmd, chỉ đổi cú pháp auth.uid()
-- ---------------------------------------------------------------------------
...
create policy "students read attendance machine photos" on public.attendance_machine_photos
  ...
  using ((is_admin() OR (EXISTS ( SELECT 1
   FROM (attendance_sessions s
     JOIN course_enrollments e ON (((e.course_id = s.course_id) AND (e.student_id = (select auth.uid())) AND (e.status = 'active'::text))))
...
create policy "students read own or graded checklist attempts" on public.checklist_attempts
  ...
  for select
  to authenticated
  using ((is_admin() OR (student_id = (select auth.uid())) OR (grader_id = (select auth.uid()))));
...
create policy "students read course checklist sessions" on public.checklist_sessions
  ...
  using ((is_admin() OR (EXISTS ( SELECT 1
   FROM course_enrollments e
  WHERE ((e.course_id = checklist_sessions.course_id) AND (e.student_id = (select auth.uid())) AND (e.status = 'active'::text))))));
...
create policy "staff post announcements" on public.class_announcements
  ...
  for insert
  to authenticated
  with check (((created_by = (select auth.uid())) AND (manages_class(class_id) OR assists_class(class_id) OR is_admin())));
...
create policy "students read class announcements" on public.class_announcements
  ...
  WHERE ((uc.user_id = (select auth.uid())) AND (uc.class_id = class_announcements.class_id) AND (uc.status = 'active'::text)))));
...
create policy "instructors read own class assignments" on public.class_instructors
  ...
  for select
  to authenticated
  using (((instructor_id = (select auth.uid())) OR is_admin()));
...
create policy "students resubmit rejected cnc drawings" on public.cnc_drawing_submissions
  ...
  for update
  to authenticated
  using (((student_id = (select auth.uid())) AND (status = 'rejected'::text)))
  with check (((student_id = (select auth.uid())) AND (status = 'pending'::text) AND (reviewed_by IS NULL) AND (reviewed_at IS NULL)));
...
create policy "students read joined courses" on public.course_offerings
  ...
  using ((is_admin() OR (EXISTS ( SELECT 1
   FROM course_enrollments e
  WHERE ((e.course_id = course_offerings.id) AND (e.student_id = (select auth.uid())))))));
...
create policy "student reads own attempts" on public.exam_attempts
  ...
  for select
  to authenticated
  using (((student_id = (select auth.uid())) OR teaches_student(student_id)));
...
create policy "assigned instructors read student results" on public.exam_results
  ...
  WHERE ((uc.user_id = exam_results.student_id) AND (ci.instructor_id = (select auth.uid()))))));
...
create policy "class members read homeroom monitors" on public.homeroom_monitors
  ...
  for select
  to authenticated
  using (((student_id = (select auth.uid())) OR is_course_member(course_id)));
...
create policy "admin sends messages" on public.messages
  ...
  for insert
  to authenticated
  with check ((is_admin() AND (sender_id = (select auth.uid()))));
...
create policy "read own messages" on public.messages
  ...
  for select
  to authenticated
  using ((is_admin() OR (recipient_id = (select auth.uid())) OR ((recipient_id IS NULL) AND ((class_name IS NULL) OR (class_name = ( SELECT profiles.class_name
   FROM profiles
...
create policy "assigned instructors read student profiles" on public.profiles
  ...
  using ((EXISTS ( SELECT 1
   FROM (user_classes uc
     JOIN class_instructors ci ON ((ci.class_id = uc.class_id)))
  WHERE ((uc.user_id = profiles.id) AND (ci.instructor_id = (select auth.uid()))))));
...
create policy "instructors read enrolled student profiles" on public.profiles
  ...
  WHERE ((e.student_id = profiles.id) AND (ci.instructor_id = (select auth.uid()))))));
...
create policy "parent updates own profile" on public.profiles
  ...
  for update
  to authenticated
  using (((id = (select auth.uid())) AND (role = 'parent'::text)))
  with check (((id = (select auth.uid())) AND (role = 'parent'::text)));
...
create policy "update own profile" on public.profiles
  ...
  with check (((id = (select auth.uid())) AND (role = 'student'::text)));
...
create policy "staff manage question bank" on public.question_bank
  ...
  using ((is_admin() OR (EXISTS ( SELECT 1
   FROM profiles p
  WHERE ((p.id = (select auth.uid())) AND (p.role = 'instructor'::text) AND (p.admin_area = 'thpt'::text))))))
...
create policy "student reads exit-quiz bank questions" on public.question_bank
  ...
  WHERE ((n.student_id = (select auth.uid())) AND (n.topic_id = question_bank.topic_id) AND (n.status = ANY (ARRAY['open'::text, 'assigned'::text, 'tutored'::text])))))));
...
create policy "read own rank_fix_attempts" on public.rank_fix_attempts
  ...
  for select
  to authenticated
  using (((student_id = (select auth.uid())) OR teaches_student(student_id) OR is_parent_of(student_id)));
...
create policy "students manage own self rubric attempt" on public.rubric_exam_attempts
  ...
  for insert
  to authenticated
  with check (((student_id = (select auth.uid())) AND (role = 'self'::text) AND (grader_id IS NULL)));
...
create policy "students update own self rubric attempt" on public.rubric_exam_attempts
  ...
  for update
  to authenticated
  using (((student_id = (select auth.uid())) AND (role = 'self'::text)))
  with check (((student_id = (select auth.uid())) AND (role = 'self'::text) AND (grader_id IS NULL)));
...
create policy "student reads own visible alerts" on public.student_alerts
  ...
  for select
  to authenticated
  using ((((student_id = (select auth.uid())) AND (status <> 'open'::text)) OR teaches_student(student_id)));
...
create policy "assigned instructors read class learning presence" on public.student_learning_presence
  ...
  using ((EXISTS ( SELECT 1
   FROM (user_classes uc
     JOIN class_instructors ci ON ((ci.class_id = uc.class_id)))
  WHERE ((uc.user_id = student_learning_presence.student_id) AND (ci.instructor_id = (select auth.uid()))))));
...
create policy "assigned instructors manage thpt attendance records" on public.thpt_attendance_records
  ...
  WHERE ((s.id = thpt_attendance_records.session_id) AND (ci.instructor_id = (select auth.uid()))))))
...
create policy "assigned instructors manage thpt attendance sessions" on public.thpt_attendance_sessions
  ...
  WHERE ((ci.class_id = thpt_attendance_sessions.class_id) AND (ci.instructor_id = (select auth.uid()))))))
  with check ((EXISTS ( SELECT 1
   FROM class_instructors ci
  WHERE ((ci.class_id = thpt_attendance_sessions.class_id) AND (ci.instructor_id = (select auth.uid()))))));
...
create policy "students read own class thpt attendance sessions" on public.thpt_attendance_sessions
  ...
  using ((EXISTS ( SELECT 1
   FROM user_classes uc
  WHERE ((uc.class_id = thpt_attendance_sessions.class_id) AND (uc.user_id = (select auth.uid()))))));
...
create policy "anyone reads public courses" on public.thpt_courses
  ...
  for select
  to public
  using (((is_public AND (status = 'active'::text)) OR (((select auth.uid()) IS NOT NULL) AND manages_class(class_id)) OR thpt_has_registration(id)));
...
create policy "staff read assigned thpt registrations" on public.thpt_registration_requests
  ...
  using ((is_admin() OR (EXISTS ( SELECT 1
   FROM class_instructors ci
  WHERE ((ci.class_id = thpt_registration_requests.requested_class_id) AND (ci.instructor_id = (select auth.uid())))))));
...
create policy "own registrations" on public.thpt_registrations
  ...
  for select
  to authenticated
  using (((student_id = (select auth.uid())) OR (registered_by = (select auth.uid())) OR ((student_id IS NOT NULL) AND is_parent_of(student_id))));
...
create policy "student registers self" on public.tutoring_registrations
  ...
  with check (((student_id = (select auth.uid())) AND (EXISTS ( SELECT 1
   FROM (tutoring_slots s
     JOIN user_classes uc ON (((uc.class_id = s.class_id) AND (uc.user_id = (select auth.uid())) AND (uc.status = 'active'::text))))
...
create policy "assistant writes own coverage" on public.tutoring_session_topics
  ...
  WHERE ((s.id = tutoring_session_topics.session_id) AND (a.user_id = (select auth.uid()))))) OR is_admin()))
  with check (((EXISTS ( SELECT 1
   FROM (ta_sessions s
     JOIN ta_assistants a ON ((a.id = s.assistant_id)))
  WHERE ((s.id = tutoring_session_topics.session_id) AND (a.user_id = (select auth.uid()))))) OR is_admin()));
...
create policy "assistant manages own slots" on public.tutoring_slots
  ...
  WHERE ((a.id = tutoring_slots.assistant_id) AND (a.user_id = (select auth.uid()))))) OR is_admin()))
  with check (((EXISTS ( SELECT 1
   FROM ta_assistants a
  WHERE ((a.id = tutoring_slots.assistant_id) AND (a.user_id = (select auth.uid()))))) OR is_admin()));
...
create policy "class reads own slots" on public.tutoring_slots
  ...
  using (((EXISTS ( SELECT 1
   FROM user_classes uc
  WHERE ((uc.user_id = (select auth.uid())) AND (uc.class_id = tutoring_slots.class_id) AND (uc.status = 'active'::text)))) OR manages_class(class_id) OR assists_class(class_id) OR is_admin()));
...
-- ============================================================================
-- ROLLBACK: chạy perf/rollback/20260925140000_perf_rls.down.sql (định nghĩa cũ,
-- sinh từ chính DB trước khi sửa). Đổi cú pháp là hai chiều nên rollback không
-- mất dữ liệu, không đổi quyền.
-- ============================================================================
