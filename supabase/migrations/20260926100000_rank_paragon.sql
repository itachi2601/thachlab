-- ============================================================================
-- feat(rank): danh vị Vô Song (Paragon) — huy hiệu độc quyền trên bậc Chí Tôn, 26/9/2026
-- ============================================================================
-- CHƯA CHẠY LÊN PRODUCTION. Chạy: bash scripts/run-migrations.sh (hoặc
-- supabase db query --linked -f supabase/migrations/20260926100000_rank_paragon.sql).
-- Chạy SAU: docs/supabase-migration-rank-daily-streak.sql và
-- docs/supabase-migration-rank-class-board.sql (hai hàm dưới đây là bản mới của
-- rank_status_of / rank_class_board trong hai file đó, chỉ thêm cờ `paragon`).
-- Idempotent — chạy lại được. Không đổi bảng, không đổi policy, không cần backfill.
--
-- Danh vị KHÔNG phải bậc thứ tám: rank_tiers / rank_student_seasons.tier_code giữ nguyên
-- ('thach_dau' vẫn là cao nhất). Cờ tính lúc đọc, không lưu:
--   paragon = đang ở Chí Tôn  AND  rank_is_paragon(học sinh)
-- rank_is_paragon = với MỌI danh hiệu enabled đang khả dụng
--   (specialist phải có chủ đề gắn — rank_title_is_active; collection/achievement luôn khả dụng),
--   học sinh đã có award ở mức cao nhất: specialist → huyen_thoai (rank 3),
--   collection/achievement → có award là đủ (các loại này chỉ có một mức).
-- Điều kiện "duy nhất trong lớp" / "giữ tới hết mùa" CHƯA đưa vào — thầy quyết sau,
-- UI hiện ghi "giữ trọn bộ tới hết mùa" như lời nhắc, không kiểm tra.
--
-- Trang web đã sẵn sàng đọc cờ này (features/rank/types.ts RankTierInfo.paragon,
-- RankBadge paragon → ParagonBadge). Chưa chạy migration thì cờ vắng → mọi thứ như cũ.
-- ============================================================================

create or replace function public.rank_is_paragon(p_student uuid)
returns boolean
language sql stable set search_path = public
as $$
  select exists (select 1 from public.rank_titles t where t.enabled)
     and not exists (
    select 1 from public.rank_titles t
    where t.enabled
      and (t.kind <> 'specialist' or public.rank_title_is_active(t.code))
      and not exists (
        select 1 from public.rank_title_awards a
        where a.student_id = p_student and a.title_code = t.code
          and public.rank_level_rank(a.level) >= case when t.kind = 'specialist' then 3 else 1 end
      )
  );
$$;
grant execute on function public.rank_is_paragon(uuid) to authenticated;

-- rank_status_of: thêm tier.paragon (bản gốc: docs/supabase-migration-rank-daily-streak.sql dòng 274–377)
...
-- rank_class_board: thêm top_week[].paragon (bản gốc: docs/supabase-migration-rank-class-board.sql dòng 15–116)
...
-- ============================================================================
-- ROLLBACK (bản đầy đủ ở perf/rollback/20260926100000_rank_paragon.down.sql):
--   supabase db query --linked -f perf/rollback/20260926100000_rank_paragon.down.sql
-- Nội dung: khôi phục rank_status_of + rank_class_board bản không có cờ paragon,
-- rồi drop function public.rank_is_paragon(uuid).
-- ============================================================================
