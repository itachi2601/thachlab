...
export async function fetchCncCourses() {
  const { data, error } = await getSupabase().from("course_offerings")
    ...
    .eq("subjects.code", "cnc").order("school_year", { ascending: false });
  if (error) throw error;
  return (data ?? []) as unknown as CourseOffering[];
}

export async function createCncCourse(input: Pick<CourseOffering, "name" | "class_label" | "school_year" | "enrollment_mode">) {
  const supabase = getSupabase();
  const [{ data: subject, error: subjectError }, { data: auth }] = await Promise.all([
    supabase.from("subjects").select("id").eq("code", "cnc").single(), supabase.auth.getUser(),
  ]);
  if (subjectError) throw subjectError;
  const joinCode = `CNC-${crypto.randomUUID().replaceAll("-", "").slice(0, 6).toUpperCase()}`;
...
export async function fetchMyActiveCncCourse(userId: string) {
  const { data, error } = await getSupabase().from("course_enrollments")
    .select("status, course_offerings!inner(id, name, class_label, school_year, status, subjects!inner(code))")
    .eq("student_id", userId).eq("course_offerings.subjects.code", "cnc")
