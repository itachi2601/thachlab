...
import { Award, ClipboardCheck, FileCheck2, Grid3x3 } from "lucide-react";
...
import TeacherRubricExamPanel from "@/components/dashboard/TeacherRubricExamPanel";
...
export default function TeacherCompetencyHub({ courseId, students }: { courseId: number; students: Student[] }) {
  const [view, setView] = useState<"approvals" | "checklist" | "rubric" | "matrix">("approvals");
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap gap-2">
      ...
      <button onClick={() => setView("rubric")} className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-bold ${view === "rubric" ? "border-orange-400/40 bg-orange-500/10 text-orange-200" : "border-white/10 text-slate-400"}`}><Award size={16} />Chấm thi thực hành tổng hợp</button>
    ...
    {view === "rubric" && <TeacherRubricExamPanel courseId={courseId} students={students.map(item => ({ id: item.id, name: item.name, className: item.className }))} />}
