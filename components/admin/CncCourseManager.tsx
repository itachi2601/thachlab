...
export default function CncCourseManager() {
  ...
  const [subjectCode, setSubjectCode] = useState<(typeof SUBJECTS)[number]["code"]>("cnc");
  ...
  const reloadCourses = useCallback(async () => {
    try { const rows = await fetchCncCourses(subjectCode); setCourses(rows); setSelectedId((current) => rows.some((row) => row.id === current) ? current : rows[0]?.id ?? null); }
    catch (error) { toast("error", errorMessage(error, "Chưa tải được danh sách khóa học.")); }
  }, [toast, subjectCode]);
  ...
  useEffect(() => { setCourses([]); setSelectedId(null); const timer = window.setTimeout(() => void reloadCourses(), 0); return () => clearTimeout(timer); }, [reloadCourses]);
  ...
  return <div className="space-y-5">
    <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#172c46] to-[#071426] p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-sky-300">CTTC · {subject.label}</p><h2 className="mt-2 font-display text-2xl font-bold text-white">Quản lý khóa học</h2><p className="mt-2 text-sm text-slate-400">Tạo khóa theo năm, gửi mã cho học sinh và duyệt danh sách tham gia.</p></div><div className="flex flex-wrap items-center gap-2"><select aria-label="Môn học" value={subjectCode} onChange={(e)=>setSubjectCode(e.target.value as (typeof SUBJECTS)[number]["code"])} className="rounded-xl border border-white/10 bg-[#080d1d] px-4 py-2.5 text-sm text-white">{SUBJECTS.map((item)=><option key={item.code} value={item.code}>{item.label}</option>)}</select><button onClick={() => setShowCreate((value) => !value)} className="flex items-center gap-2 rounded-full bg-blue-600 px-5 py-2.5 text-sm font-bold text-white"><Plus size={16}/> Tạo khóa mới</button></div></div></section>
    {showCreate && <section className="rounded-2xl border border-blue-400/20 bg-blue-500/5 p-5"><div className="grid gap-3 md:grid-cols-2"><input value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})} placeholder={`Tên khóa: ${subject.joinPrefix} K25A`} className="rounded-xl border border-white/10 bg-[#080d1d] px-4 py-3 text-white"/><input value={form.class_label} onChange={(e)=>setForm({...form,class_label:e.target.value})} placeholder="Lớp: CĐ CK 25A" className="rounded-xl border border-white/10 bg-[#080d1d] px-4 py-3 text-white"/><input value={form.school_year} onChange={(e)=>setForm({...form,school_year:e.target.value})} className="rounded-xl border border-white/10 bg-[#080d1d] px-4 py-3 text-white"/><select value={form.enrollment_mode} onChange={(e)=>setForm({...form,enrollment_mode:e.target.value as EnrollmentMode})} className="rounded-xl border border-white/10 bg-[#080d1d] px-4 py-3 text-white"><option value="approval">Giáo viên duyệt</option><option value="automatic">Tự động duyệt</option><option value="closed">Đóng đăng ký</option></select></div><button disabled={busy||!form.name.trim()} onClick={async()=>{setBusy(true);try{await createCncCourse(form,subjectCode,subject.joinPrefix);setShowCreate(false);setForm({...form,name:"",class_label:""});await reloadCourses();toast("success","Đã tạo khóa học.");}catch(error){toast("error",errorMessage(error,"Chưa tạo được khóa học."));}finally{setBusy(false);}}} className="mt-4 rounded-full bg-blue-600 px-5 py-2 text-sm font-bold text-white disabled:opacity-40">Lưu khóa</button></section>}
    <div className="grid gap-5 lg:grid-cols-[280px_1fr]"><aside className="rounded-2xl border border-white/10 bg-[#0B1020] p-4"><h3 className="font-semibold text-white">Các khóa · {subject.label}</h3><div className="mt-3 space-y-2">{courses.map((course)=><button key={course.id} onClick={()=>setSelectedId(course.id)} className={`w-full rounded-xl border p-3 text-left ${selectedId===course.id?"border-blue-400 bg-blue-500/10":"border-white/5"}`}><strong className="block text-sm text-white">{course.name}</strong><small className="text-slate-400">{course.school_year} · {course.class_label}</small></button>)}{courses.length===0&&<p className="text-sm text-slate-400">Chưa có khóa học.</p>}</div></aside>
