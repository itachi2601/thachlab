...
export default function ExamEditor({exam,initialQuestions,initialTitle,cncBank,onDone}:{exam:Exam|null;initialQuestions?:MultipleChoiceQuestion[];initialTitle?:string;cncBank:{key:string;label:string};onDone:()=>void}){
  ...
  return <div className="space-y-6">
    ...
    {(Object.keys(TYPE_META) as QuestionType[]).map(type=>{const entries=questions.map((q,index)=>({q,index})).filter(item=>item.q.type===type);const meta=TYPE_META[type];const Icon=meta.icon;return <section key={type} className="rounded-3xl border border-white/10 bg-panel p-5">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-blue-300"><Icon size={19}/></span><div><h2 className="admin-h2">{meta.label}</h2><p className="text-xs text-slate-500">{meta.description} · {entries.length} câu</p></div></div><button type="button" onClick={()=>setQuestions(current=>[...current,newQuestion(type)])} className="inline-flex items-center gap-1 rounded-full border border-blue-400/25 bg-blue-500/10 px-4 py-2 text-xs font-bold text-blue-200"><Plus size={14}/> Thêm câu {meta.label.toLowerCase()}</button></div>
    ...
    <div className="sticky bottom-4 flex flex-wrap gap-3 rounded-2xl border border-white/10 bg-[#080D1A]/95 p-4 shadow-2xl backdrop-blur"><button onClick={save} disabled={busy} className="rounded-full bg-primary px-6 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{busy?"Đang lưu…":"Lưu toàn bộ câu hỏi"}</button><button onClick={onDone} disabled={busy} className="rounded-full px-5 py-2.5 text-sm text-slate-400 hover:text-white">Quay lại</button></div>
  </div>;
}
