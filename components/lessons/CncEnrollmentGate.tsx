...
export default function CncEnrollmentGate(){
  ...
  if(profile?.role==="admin")return <CncCourseWorkspace embedded/>;
  if(!loaded)return <Shell>Đang tải thông tin khóa…</Shell>;
  if(access?.status==="active")return <><div className="mx-5 mt-5 flex items-center gap-2 rounded-2xl border border-emerald-400/25 bg-emerald-500/10 px-5 py-3 text-sm text-emerald-200"><CheckCircle2 size={18}/><strong>{access.course.name}</strong><span>· {access.course.school_year}</span></div><CncCourseWorkspace embedded courseId={access.course.id}/></>;
