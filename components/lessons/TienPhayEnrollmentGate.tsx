...
import { CheckCircle2, Clock3, KeyRound, LogIn, ShieldCheck, Wrench } from "lucide-react";
...
const SUBJECT_CODE = "tien-phay";

type Access = Awaited<ReturnType<typeof fetchMyActiveCncCourse>>;
export default function TienPhayEnrollmentGate(){
  ...
  const reload=useCallback(async()=>{if(!session)return;try{setAccess(await fetchMyActiveCncCourse(session.user.id,SUBJECT_CODE));}catch(e){setMessage(e instanceof Error?e.message:"Không kiểm tra được khóa học.");}finally{setLoaded(true);}},[session]);
  ...
  if(!session)return <Shell><LogIn className="mx-auto text-blue-300" size={36}/><h1 className="mt-4 text-2xl font-bold text-white">Đăng nhập để học Tiện – Phay</h1><p className="mt-2 text-sm text-slate-400">Đăng nhập rồi nhập mã khóa do giáo viên cung cấp.</p><Link href="/dang-nhap" className="mt-5 inline-flex rounded-full bg-blue-600 px-6 py-2.5 text-sm font-bold text-white">Đăng nhập</Link></Shell>;
  if(profile?.role==="admin")return <Shell><Wrench className="mx-auto text-orange-300" size={36}/><h1 className="mt-4 text-2xl font-bold text-white">Quản lý khóa Tiện – Phay ở Dashboard</h1><p className="mt-2 text-sm text-slate-400">Chọn môn &quot;Tiện – Phay truyền thống&quot; trong Dashboard giáo viên để điểm danh và theo dõi tình trạng máy.</p><Link href="/dashboard" className="mt-5 inline-flex rounded-full bg-blue-600 px-6 py-2.5 text-sm font-bold text-white">Vào Dashboard</Link></Shell>;
  if(!loaded)return <Shell>Đang tải thông tin khóa…</Shell>;
  if(access?.status==="active")return <><div className="mx-5 mt-5 flex items-center gap-2 rounded-2xl border border-emerald-400/25 bg-emerald-500/10 px-5 py-3 text-sm text-emerald-200"><CheckCircle2 size={18}/><strong>{access.course.name}</strong><span>· {access.course.school_year}</span></div><div className="mx-auto max-w-3xl px-5 pb-16"><StudentAttendancePanel courseId={access.course.id} studentId={session.user.id} workshop={getSubject(SUBJECT_CODE).workshop}/></div></>;
  ...
  return <Shell><KeyRound className="mx-auto text-blue-300" size={38}/><h1 className="mt-4 text-2xl font-bold text-white">Tham gia khóa Tiện – Phay truyền thống</h1><p className="mt-2 text-sm text-slate-400">Nhập mã khóa để gửi yêu cầu đến giáo viên.</p><form className="mx-auto mt-6 flex max-w-md flex-col gap-3 sm:flex-row" onSubmit={async(e)=>{e.preventDefault();if(!code.trim())return;setBusy(true);setMessage("");try{await requestEnrollment(code);await reload();}catch(err){setMessage(err instanceof Error?err.message:"Mã khóa không hợp lệ.");}finally{setBusy(false);}}}><input value={code} onChange={(e)=>setCode(e.target.value.toUpperCase())} placeholder="TP-A1B2C3" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-center font-mono uppercase text-white"/><button disabled={busy} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white">{busy?"Đang gửi…":"Gửi yêu cầu"}</button></form>{message&&<p className="mt-4 text-sm text-amber-200">{message}</p>}</Shell>;
}
