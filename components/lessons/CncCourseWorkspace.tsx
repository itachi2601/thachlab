...
export default function CncCourseWorkspace({ embedded = false, courseId, initialLessonId }: { embedded?: boolean; courseId?: number; initialLessonId?: string }) {
  ...
  const recordQuiz = useCallback((quizLessonId: string, assessmentId: string, quizScore: number, total: number, didPass: boolean) => {
    ...
    setAssessmentAttempts((current) => { const key = `${quizLessonId}:${assessmentId}`; return { ...current, [key]: (current[key] ?? 0) + 1 }; });
  ...
  useEffect(() => {
    ...
    fetchCncLearningRecords(courseId, session.user.id).then((rows) => {
      if (!cancelled) {
        ...
        setAssessmentAttempts(Object.fromEntries(rows.map((row) => [`${row.lesson_id}:${row.assessment_id}`, row.attempt_count])));
      }
    }).catch(() => undefined);
    return () => { cancelled = true; };
  }, [courseId, session?.user.id, viewerProfile?.role]);
    ...
    <div className={`cnc-shell lesson-shell lesson-shell--cttc ${embedded ? "cnc-embedded" : ""}`}>
      ...
      <div className="cnc-workspace cnc-accordion-workspace">
        <section className="cnc-course-outline">
          ...
          {validInitialLessonId && <Link href="/lop-hoc/cnc" className="lesson-link mb-4">← Tất cả bài học</Link>}
...
function StandardTest({ lessonId, lessonTitle, quizBanks, onPassed, onResult, showHeading = true }: { lessonId: string; lessonTitle: string; quizBanks: Record<string, QuizQuestion[]>; onPassed: () => void; onResult?: (score:number,total:number,passed:boolean)=>void; showHeading?: boolean }) {
  ...
  if (bankKey && !quiz) return <p className="lesson-muted">Đang tải câu hỏi…</p>;
...
function ProgrammingTestSuite({ lessonId, lessonTitle, courseId, quizBanks, onPassed, onResult, passed, attempts }: { lessonId: "lesson-2" | "lesson-3"; lessonTitle: string; courseId?: number; quizBanks: Record<string, QuizQuestion[]>; onPassed: () => void; onResult?: (assessmentId:string,score:number,total:number,passed:boolean)=>void; passed: Set<string>; attempts: Record<string, number> }) {
  ...
  return <div className="cnc-programming-suite space-y-5">
    ...
    <div className="cnc-programming-tabs grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{levels.map((item)=><button key={item.id} type="button" onClick={()=>setLevel(item.id)} className={level===item.id?"active":""}><ExerciseStatusBadge status={exerciseStatus(passed,attempts,lessonId,item.id)} attempts={attempts[`${lessonId}:${item.id}`]}/><strong>{item.name}</strong><small>{item.note}</small></button>)}</div>
    {level === "exercise-1" && (lessonQuiz ? <RandomMcQuiz quiz={lessonQuiz} kicker="BÀI TẬP 1 · NHẬN BIẾT CÂU LỆNH" title="Nhận biết câu lệnh G-code/M-code" onResult={(score,total,didPass)=>onResult?.("exercise-1",score,total,didPass)} /> : <p className="lesson-muted">Đang tải câu hỏi…</p>)}
    {level === "exercise-2" && (translateQuiz ? <RandomMcQuiz quiz={translateQuiz} kicker="BÀI TẬP 2 · DỊCH LỆNH (TRẮC NGHIỆM)" title="Đọc hiểu và dịch block lệnh" onResult={(score,total,didPass)=>onResult?.("exercise-2",score,total,didPass)} /> : <p className="lesson-muted">Đang tải câu hỏi…</p>)}
    {level === "exercise-3-short" && (translateQuiz ? <ShortAnswerTranslationQuiz machine={machine} bank={translateQuiz} onResult={(score,total,didPass)=>onResult?.("exercise-3-short",score,total,didPass)} /> : <p className="lesson-muted">Đang tải câu hỏi…</p>)}
  ...
  return <>
    ...
    <div className="cnc-quiz">
      <fieldset className={checked && grading && !grading.correct ? "wrong" : ""}>
        ...
        <pre className="cnc-code-block mt-1 text-center">{question.question}</pre>
        <textarea
          className="cnc-answer-input"
          rows={3}
...
function MillingFillBlankExercise({onResult}:{onResult?:(score:number,total:number,passed:boolean)=>void}){
  ...
  return <div className="cnc-exercise-card"><span className="cnc-eyebrow">BÀI TẬP 3 · ĐIỀN KHUYẾT CHƯƠNG TRÌNH</span><h3>{exercise.title}</h3><p>Quan sát bản vẽ và thông số công nghệ, sau đó điền các giá trị còn thiếu trong chương trình.</p><div className="cnc-exercise-figure"><Image src={imageMap[level]} alt="Bản vẽ chi tiết milling exercise" width={1048} height={956} className="h-auto w-full" unoptimized priority/></div><LevelPicker value={level} onChange={setLevel}/><FillBlankLevel key={`mill-${level}`} level={selected} onResult={onResult}/></div>;
}
...
function TurningFillBlankExercise({onResult}:{onResult?:(score:number,total:number,passed:boolean)=>void}){return <div className="cnc-exercise-card"><span className="cnc-eyebrow">BÀI TẬP 3 · ĐIỀN KHUYẾT CHƯƠNG TRÌNH TIỆN</span><h3>Hoàn thành chương trình gia công trục bậc – côn</h3><p>Quan sát bản vẽ và thông số công nghệ, sau đó điền các giá trị còn thiếu trong chương trình.</p><div className="cnc-exercise-figure"><Image src="/images/cnc/bai-3-tien-dien-khuyet.png" alt="Bản vẽ chi tiết tiện trục bậc và côn" width={1054} height={876} className="h-auto w-full" unoptimized priority/></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="cnc-exercise-brief"><strong>Tiện thô</strong><ul><li>Dao T0202 · n₁ = 800 vòng/phút</li><li>S₁ = 0,2 mm/vòng · t₁ = 1 mm</li></ul></div><div className="cnc-exercise-brief"><strong>Tiện tinh</strong><ul><li>Dao T0404 · n₂ = 1000 vòng/phút</li><li>S₂ = 0,1 mm/vòng · t₂ = 0,5 mm</li></ul></div></div><FillBlankLevel level={{part_ref:"Phôi Ø20 dài 60 mm; biên dạng Ø10 × 3, côn đến Ø16 tại Z-15 và đoạn Ø16 đến Z-25.",part_image_note:"Gốc W đặt tại tâm mặt đầu chi tiết, Z0 ở mặt đầu bên phải.",program_with_blanks:turningFillBlankProgram,blanks:turningFillBlankAnswers}} onResult={onResult}/></div>}

function InputBlank({id,values,setValues}:{id:string;values:Record<string,string>;setValues:React.Dispatch<React.SetStateAction<Record<string,string>>>}){return <input aria-label={`Giá trị ${id}`} value={values[id]??""} onChange={event=>setValues(current=>({...current,[id]:event.target.value}))} className="cnc-code-input"/>}
...
function FillBlankLevel({level,onResult}:{level:FillLevel;onResult?:(score:number,total:number,passed:boolean)=>void}){const[values,setValues]=useState<Record<string,string>>({});const[submitted,setSubmitted]=useState(false);const normalized=(value:string)=>value.trim().toUpperCase().replace(/^G(?=\d)/,"");const score=level.blanks.filter(item=>normalized(values[item.id]??"")===normalized(item.answer)).length;return <><div className="cnc-exercise-brief mt-4"><strong>Chi tiết gia công</strong><p>{level.part_ref}</p><small>Gợi ý bản vẽ: {level.part_image_note}</small></div><div className="cnc-code-block mt-4">{level.program_with_blanks.map((line,index)=><div key={index} className="whitespace-nowrap">{line.split(/___\[([^\]]+)\]___/g).map((part,i)=>i%2?<InputBlank key={part} id={part} values={values} setValues={setValues}/>:<span key={i}>{part}</span>)}</div>)}</div><button className="cnc-submit" disabled={Object.keys(values).length<level.blanks.length} onClick={()=>{setSubmitted(true);onResult?.(score,level.blanks.length,score===level.blanks.length)}}>Kiểm tra {level.blanks.length} ô trống</button>{submitted&&<div className={`cnc-check-result ${score===level.blanks.length?"passed":""}`}><strong>{score===level.blanks.length?"Đạt — Tất cả giá trị đều chính xác.":`Đúng ${score}/${level.blanks.length} ô.`}</strong>{score<level.blanks.length&&<ul>{level.blanks.filter(item=>normalized(values[item.id]??"")!==normalized(item.answer)).map(item=><li key={item.id}>{item.id}: {item.note}</li>)}</ul>}</div>}</>}

function LevelPicker({value,onChange}:{value:"de"|"kho";onChange:(value:"de"|"kho")=>void}){return <div className="cnc-level-picker"><button type="button" onClick={()=>onChange("de")} className={value==="de"?"active":""}>Mức dễ</button><button type="button" onClick={()=>onChange("kho")} className={value==="kho"?"active":""}>Mức khó</button></div>}
...
function ProgramFromDrawingExercise({machine,onResult}:{machine:"turn"|"mill";onResult?:(score:number,total:number,passed:boolean)=>void}){const exercise=(machine==="turn"?turningExercises:millingExercises).exercises[1] as {title:string;levels:(ProgramLevel&{level:string})[]};const[level,setLevel]=useState<"de"|"kho">("de");const selected=exercise.levels.find(item=>item.level===level)??exercise.levels[0];return <div className="cnc-exercise-card"><span className="cnc-eyebrow">BÀI TẬP 4 · VIẾT CHƯƠNG TRÌNH TỪ BẢN VẼ</span><h3>{exercise.title}</h3>{selected.image&&<div className="cnc-exercise-figure"><Image src={selected.image} alt="Bản vẽ chi tiết bài tập 4" width={1876} height={1580} className="h-auto w-full" unoptimized priority/></div>}{exercise.levels.length>1&&<LevelPicker value={level} onChange={setLevel}/>}<ProgramLevelChallenge key={`${machine}-${level}`} machine={machine} level={selected} onResult={onResult}/></div>}

function ProgramLevelChallenge({machine,level,onResult}:{machine:"turn"|"mill";level:ProgramLevel;onResult?:(score:number,total:number,passed:boolean)=>void}){const[code,setCode]=useState(machine==="turn"?"G90 G95 G18\nT0202\n":"G21 G90 G94 G17\nG54\nT01 M06\nG43 H01 Z50\n");const[result,setResult]=useState<string[]|null>(null);function check(){const value=code.toUpperCase().replace(/\s+/g," ");const tests:[boolean,string][]=[[machine==="mill"?value.includes("G54"):true,"Thiếu khai báo gốc phôi G54"],[/M0?3/.test(value),"Thiếu lệnh quay trục chính M03"],[/M0?5/.test(value),"Thiếu lệnh dừng trục chính M05"],[/M0?30/.test(value),"Thiếu lệnh kết thúc M30"],...level.expected_key_coordinates.map(point=>{const coords=[`X${point.X}`,point.Y!==undefined?`Y${point.Y}`:`Z${point.Z}`];return [coords.every(coord=>new RegExp(`${coord.replace("-","\\-")}(?:\\.0+)?(?:[\\s;]|$)`).test(value)),`Thiếu hoặc sai tọa độ ${point.point}: ${coords.join(" ")}`] as [boolean,string]})];if(machine==="turn")tests.push([/T0?202/.test(value),"Thiếu dao tiện thô T0202"],[/T0?404/.test(value),"Thiếu dao tiện tinh T0404"],[/G95/.test(value),"Thiếu chế độ chạy dao G95 (mm/vòng)"],[/G(?:73|71|0?1)/.test(value),"Thiếu chu trình G73/G71 hoặc nội suy G01"]);else tests.push([/T0?1\s+M0?6/.test(value),"Thiếu T01 M06"],[/G43\s+H0?1/.test(value),"Thiếu bù chiều dài G43 H01"],[/G4[12]/.test(value),"Thiếu bù bán kính G41/G42"],[/G40/.test(value),"Thiếu hủy bù bán kính G40"]);if(level.part_ref.includes("ren"))tests.push([/T0?202/.test(value),"Thiếu dao tiện ren T0202"],[/G(?:92|76)/.test(value),"Thiếu chu trình tiện ren G92/G76"],[/F1(?:\.5|,5)/.test(value),"Sai hoặc thiếu bước ren F1.5"]);if(level.part_ref.includes("cung"))tests.push([/G0?[23]/.test(value),"Thiếu nội suy cung G02/G03"],[/(?:I|J|R)-?\d/.test(value),"Thiếu thông số I, J hoặc R"]);const missing=tests.filter(([ok])=>!ok).map(([,message])=>message);setResult(missing);onResult?.(tests.length-missing.length,tests.length,missing.length===0)}return <div className="mt-4 grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><div><div className="cnc-exercise-brief"><strong>Đề bài</strong><p>{level.part_ref}</p></div><h4>Các yêu cầu cần đạt</h4><ul>{level.required_elements.map(item=><li key={item}>{item}</li>)}</ul><h4>Rubric giáo viên</h4>{level.teacher_rubric.map(item=><div key={item.criterion} className="cnc-exercise-rubric-row"><span>{item.criterion}</span><strong>{item.weight}</strong></div>)}</div><div className="cnc-code-panel"><span className="cnc-eyebrow">NC CODE CHECKER · {machine==="turn"?"FANUC 21T":"FANUC 21M"}</span><textarea value={code} onChange={event=>{setCode(event.target.value);setResult(null)}} spellCheck={false} className="cnc-code-editor mt-4 min-h-96"/><button type="button" onClick={check} className="cnc-submit">Kiểm tra chương trình</button>{result&&<div className={`cnc-check-result ${result.length===0?"passed":""}`}>{result.length===0?<strong>Đạt — Chương trình đáp ứng đầy đủ tiêu chí tự động.</strong>:<><strong>Còn {result.length} tiêu chí cần sửa:</strong><ul>{result.map(item=><li key={item}>{item}</li>)}</ul></>}</div>}</div></div>}

function SimulationEvidenceExercise({machine,...props}:{machine:"turn"|"mill";lessonId:string;lessonTitle:string;courseId?:number;onApproved:(lessonId:string)=>void}){const exercise=(machine==="turn"?turningExercises:millingExercises).exercises[2];return <div><div className="cnc-exercise-card mb-5"><span className="cnc-eyebrow">BÀI TẬP 5 · MÔ PHỎNG VÀ KIỂM CHỨNG</span><h3>{exercise.title}</h3><p>{exercise.instructions}</p><h4>Hồ sơ cần nộp</h4><ul>{exercise.submission_requirements?.map(item=><li key={item}>{item}</li>)}</ul><h4>Rubric chấm của giáo viên</h4>{exercise.teacher_rubric?.map(item=><div key={item.criterion} className="cnc-exercise-rubric-row"><span>{item.criterion}</span><strong>{item.weight}</strong></div>)}</div><DrawingSubmissionTest {...props}/></div>}
...
function MillingDrawingCodeChallenge({onResult}:{onResult?:(score:number,total:number,passed:boolean)=>void}) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><div className="cnc-exercise-card"><span className="cnc-eyebrow">DẠNG 3 · VIẾT CHƯƠNG TRÌNH TỪ BẢN VẼ</span><h3>Phay biên dạng chữ nhật 60 × 40</h3><p>Gốc phôi tại góc trái dưới; chiều sâu Z = −2 mm. Viết quỹ đạo theo chiều kim đồng hồ.</p><svg viewBox="0 0 360 260" className="cnc-exercise-figure mt-4" role="img" aria-label="Bản vẽ phay hình chữ nhật 60 nhân 40 milimét"><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="4" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="#71808c"/></marker></defs><rect x="80" y="55" width="210" height="140" fill="#fff" stroke="#17232c" strokeWidth="4"/><circle cx="80" cy="195" r="6" fill="#e85d24"/><text x="54" y="220" fill="#e85d24" fontSize="13" fontWeight="700">G54 (0,0)</text><line x1="80" y1="225" x2="290" y2="225" stroke="#71808c" markerStart="url(#arrow)" markerEnd="url(#arrow)"/><text x="175" y="248" fill="#52616d" fontSize="14">60 mm</text><line x1="320" y1="55" x2="320" y2="195" stroke="#71808c" markerStart="url(#arrow)" markerEnd="url(#arrow)"/><text x="330" y="130" fill="#52616d" fontSize="14" transform="rotate(90 330 130)">40 mm</text></svg></div><div className="cnc-code-panel"><div className="flex items-center justify-between"><div><span className="cnc-eyebrow">NC CODE CHECKER · THỬ NGHIỆM</span><h3>Nhập chương trình FANUC 21M</h3></div><span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-300">9 tiêu chí</span></div><textarea value={code} onChange={(event)=>{setCode(event.target.value);setResult(null)}} spellCheck={false} className="cnc-code-editor mt-4 min-h-72"/><button type="button" onClick={checkCode} className="cnc-submit">Kiểm tra chương trình</button>{result&&<div className={`cnc-check-result ${result.length===0?"passed":""}`}>{result.length===0?<strong>Đạt — Chương trình có đủ lệnh và đi qua đúng 4 đỉnh của biên dạng.</strong>:<><strong>Còn {result.length} tiêu chí cần sửa:</strong><ul>{result.map((item)=><li key={item}>{item}</li>)}</ul></>}</div>}</div></div>;
}

function TurningDrawingCodeChallenge({onResult}:{onResult?:(score:number,total:number,passed:boolean)=>void}){const starter="G21 G90\nT0101\nG97 S1200 M03\nG00 X42 Z2\n";const[code,setCode]=useState(starter);const[result,setResult]=useState<string[]|null>(null);function checkCode(){const value=code.toUpperCase().replace(/\s+/g," ");const checks=[[/G21/,"Thiếu G21"],[/G90/,"Thiếu G90"],[/T0?101/,"Thiếu gọi dao T0101"],[/M0?3/,"Thiếu M03"],[/X40(?:\.0+)?\s+Z0(?:\.0+)?/,"Chưa đến điểm X40 Z0"],[/X30(?:\.0+)?\s+Z-20(?:\.0+)?/,"Chưa tiện đến X30 Z−20"],[/X40(?:\.0+)?\s+Z-35(?:\.0+)?/,"Chưa tạo bậc X40 Z−35"],[/M30/,"Thiếu M30"]] as const;const missing=checks.filter(([pattern])=>!pattern.test(value)).map(([,message])=>message);setResult(missing);onResult?.(checks.length-missing.length,checks.length,missing.length===0);}return <div className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><div className="cnc-exercise-card"><span className="cnc-eyebrow">DẠNG 3 · VIẾT CHƯƠNG TRÌNH TỪ BẢN VẼ</span><h3>Tiện trục bậc Ø40/Ø30</h3><p>Gốc Z0 tại mặt đầu; đoạn Ø30 dài 20 mm, tổng chiều dài 35 mm.</p><svg viewBox="0 0 380 240" className="cnc-exercise-figure mt-4" role="img" aria-label="Bản vẽ tiện trục bậc đường kính 40 và 30"><path d="M55 60 H300 V90 H210 V150 H300 V180 H55 Z" fill="#fff" stroke="#17232c" strokeWidth="4"/><line x1="55" y1="120" x2="325" y2="120" stroke="#94a0a9" strokeDasharray="7 5"/><text x="220" y="82" fill="#52616d" fontSize="14">Ø40</text><text x="120" y="112" fill="#e85d24" fontSize="14">Ø30 × 20</text><text x="210" y="215" fill="#52616d" fontSize="14">L = 35 mm</text><circle cx="300" cy="120" r="5" fill="#e85d24"/><text x="306" y="115" fill="#e85d24" fontSize="12">Z0</text></svg></div><div className="cnc-code-panel"><span className="cnc-eyebrow">NC CODE CHECKER · FANUC 21T</span><h3>Nhập chương trình tiện</h3><textarea value={code} onChange={(event)=>{setCode(event.target.value);setResult(null)}} spellCheck={false} className="cnc-code-editor mt-4 min-h-72"/><button type="button" onClick={checkCode} className="cnc-submit">Kiểm tra chương trình</button>{result&&<div className={`cnc-check-result ${result.length===0?"passed":""}`}>{result.length===0?<strong>Đạt — chương trình đi qua đủ các điểm biên dạng trục bậc.</strong>:<><strong>Còn {result.length} tiêu chí cần sửa:</strong><ul>{result.map((item)=><li key={item}>{item}</li>)}</ul></>}</div>}</div></div>}
...
type ExerciseStatus="passed"|"attempted"|"none";
function exerciseStatus(passed:Set<string>,attempts:Record<string,number>,lessonId:string,assessmentId:string):ExerciseStatus{
  const key=`${lessonId}:${assessmentId}`;
  if(passed.has(key))return "passed";
  if((attempts[key]??0)>0)return "attempted";
  return "none";
}
function ExerciseStatusBadge({status,attempts}:{status:ExerciseStatus;attempts?:number}){
  if(status==="passed")return <span className="cnc-exercise-badge passed"><CheckCircle2 size={11}/>Đã đạt</span>;
  if(status==="attempted")return <span className="cnc-exercise-badge attempted">{attempts} lần · chưa đạt</span>;
  return null;
}
  ...
  return <>
      ...
      {sections.map((section) => <section key={section.id}>
        ...
        {section.items.map((item) => {
          ...
          return <label key={item.id} className={`${checked[item.id] ? "checked" : ""} ${critical ? "critical" : ""}`}>
            <input type="checkbox" disabled={!enabled} checked={Boolean(checked[item.id])} onChange={(event) => { setChecked((current) => ({ ...current, [item.id]: event.target.checked })); setSubmittedScore(null); }} />
            <i><Check size={14} /></i>
    ...
    <section className="cnc-zero-tolerance">
      ...
      {zeroTolerance.map((item, index) => <label key={item}>
        <input type="checkbox" disabled={!enabled} checked={Boolean(zeroToleranceClear[index])} onChange={(event) => { setZeroToleranceClear((current) => ({ ...current, [index]: event.target.checked })); setSubmittedScore(null); }} />
