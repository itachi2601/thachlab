    ...
    <div className="mt-3 grid grid-cols-5 items-start gap-1 sm:gap-3">
      {items.map((item,index)=>{const state=states[item.id];const Icon=icons[item.id];const unlocked=state.approved;const waiting=state.eligible&&!unlocked;return <div key={item.id} className="flex shrink-0 snap-start items-start">
        <Link href={`/lop-hoc/cnc/?bai=${item.lessonId}`} title={`${item.permission} — bấm để vào học`} className="group relative block min-w-0 flex-1 text-center">
          <div className={`relative z-10 mx-auto grid h-11 w-11 place-items-center rounded-full p-[2px] transition group-hover:scale-105 min-[390px]:h-12 min-[390px]:w-12 sm:h-16 sm:w-16 ${unlocked?"bg-gradient-to-br from-amber-300 via-orange-400 to-cyan-400 shadow-[0_0_24px_rgba(34,211,238,.28)]":waiting?"bg-gradient-to-br from-amber-500/70 to-orange-500/40":"bg-white/10"}`}>
          ...
          <strong className={`mt-1.5 block text-[8px] leading-[10px] underline-offset-2 group-hover:underline min-[390px]:text-[9px] sm:mt-2 sm:text-[10px] sm:leading-3 ${unlocked?"text-white":waiting?"text-amber-200":"text-slate-600"}`}>{item.title.replace(/ (tiện|phay)$/i,"")}</strong>
