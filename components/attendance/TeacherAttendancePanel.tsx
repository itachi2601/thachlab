...
import { closeAttendanceSession, createAttendanceSession, fetchAttendanceRecords, fetchAttendanceSessions, setAttendanceRecord, setAttendanceRecordBonus, setAttendanceRecordMachine, setAttendanceRecordNote, setMachineEditOverride, updateAttendanceSessionNote, type AttendanceRecord, type AttendanceSession, type AttendanceStatus } from "@/services/course-attendance";
...
export default function TeacherAttendancePanel({courseId,students}:{courseId:number;students:Student[]}){
  ...
  async function toggleMachineOverride(){if(!selected)return;setBusy(true);try{await setMachineEditOverride(selected.id,!selected.machine_edit_override);await loadSessions()}catch(cause){setError(cause instanceof Error?cause.message:"Không đổi được trạng thái mở khoá chọn máy.")}finally{setBusy(false)}}
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1.3fr_1fr]">
          <div className="rounded-2xl border border-cyan-400/20 bg-gradient-to-br from-cyan-500/10 to-blue-500/5 p-5">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <button disabled={busy} onClick={()=>void toggleMachineOverride()} title="Cho phép sinh viên chọn/đổi máy dù đã khoá phiên hoặc hết 6 tiếng" className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold disabled:opacity-40 ${selected.machine_edit_override?"border-orange-400/40 bg-orange-500/10 text-orange-200":"border-white/10 text-slate-300"}`}><Unlock size={14}/>{selected.machine_edit_override?"Đang mở khoá chọn máy":"Mở khoá chọn máy"}</button>
            </div>
          </div>
