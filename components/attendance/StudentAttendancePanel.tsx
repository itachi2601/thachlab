...
function MachineDutyPanel({sessionId,machineCode,open,machines,machinesError,onMachineChosen}:{sessionId:number;machineCode:string|null;open:boolean;machines:Machine[];machinesError:string;onMachineChosen:()=>void}){
  ...
  if(!machineCode||editing)return <div className="mt-3 rounded-xl border border-orange-400/20 bg-orange-500/5 p-3">
    ...
    {open?(machinesError?<p className="mt-2 text-xs text-red-300">Không tải được danh sách máy: {machinesError}. Thử tải lại trang hoặc báo giáo viên.</p>
      :machines.length===0?<p className="mt-2 text-xs text-amber-300">Chưa có danh sách máy trong hệ thống. Báo giáo viên để cấu hình.</p>
