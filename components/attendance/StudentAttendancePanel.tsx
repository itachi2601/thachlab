...
import { TURN_MACHINE_CODES, MILL_MACHINE_CODES, selectAttendanceMachine, isFirstOnMachine, fetchAttendanceMachinePhotos, submitAttendanceMachinePhoto, createAttendanceMachinePhotoUrl, type AttendanceMachinePhoto, type MachineCheckpoint } from "@/services/attendance-machine";

const labels:Record<string,string>={present:"Có mặt",late:"Đi muộn",excused:"Vắng có phép",absent:"Vắng không phép"};
function sessionOf(row:{attendance_sessions:unknown}){const value=row.attendance_sessions as {id:number;title:string;session_date:string;starts_at:string;status:string;machine_edit_override:boolean}|{id:number;title:string;session_date:string;starts_at:string;status:string;machine_edit_override:boolean}[];return Array.isArray(value)?value[0]:value}

export default function StudentAttendancePanel({courseId,studentId}:{courseId:number;studentId:string}){const[code,setCode]=useState("");const[rows,setRows]=useState<Awaited<ReturnType<typeof fetchMyAttendance>>>([]);const[message,setMessage]=useState("");const[busy,setBusy]=useState(false);const[expanded,setExpanded]=useState(false);const load=useCallback(()=>fetchMyAttendance(courseId,studentId).then(setRows).catch(()=>undefined),[courseId,studentId]);useEffect(()=>{void load()},[load]);const attended=rows.filter(row=>row.status==="present"||row.status==="late").length;const rate=rows.length?Math.round(attended/rows.length*100):0;const visibleRows=expanded?rows:rows.slice(0,4);const latest=rows[0];const reopenedRows=rows.filter(row=>row!==latest&&(row.status==="present"||row.status==="late")&&sessionOf(row)?.machine_edit_override);async function submit(){setBusy(true);try{const result=await submitAttendanceCode(courseId,code);setMessage(`${result.status==="late"?"Điểm danh muộn":"Điểm danh thành công"}: ${result.session_title}`);setCode("");await load()}catch(cause){setMessage(cause instanceof Error?cause.message:"Không thể điểm danh.")}finally{setBusy(false)}}return <section className="mt-5 rounded-2xl border border-cyan-400/15 bg-cyan-500/5 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><CalendarCheck size={18} className="text-cyan-300"/><strong className="text-white">Điểm danh offline</strong></div><p className="mt-1 text-xs text-slate-400">Đã tham dự {attended}/{rows.length} buổi · {rate}%</p></div><div className="flex gap-2"><input inputMode="numeric" maxLength={6} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,""))} placeholder="Mã 6 số" className="w-full min-w-0 flex-1 rounded-xl border border-white/10 bg-[#080d1d] px-3 py-2 font-mono text-sm tracking-wider text-white sm:w-32 sm:flex-none"/><button disabled={busy||code.length!==6} onClick={submit} className="shrink-0 rounded-xl bg-cyan-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-40">Xác nhận</button></div></div>{message&&<p className="mt-3 rounded-lg bg-black/20 p-2 text-xs text-cyan-100">{message}</p>}
  {latest&&(latest.status==="present"||latest.status==="late")&&<MachineDutyPanel sessionId={latest.session_id} machineCode={latest.machine_code} checkedInAt={latest.checked_in_at??undefined} sessionClosed={sessionOf(latest)?.status==="closed"} override={Boolean(sessionOf(latest)?.machine_edit_override)} onMachineChosen={load}/>}
  {reopenedRows.map(row=>{const session=sessionOf(row);return <div key={`reopen-${row.session_id}`}><p className="mt-4 text-xs font-bold text-orange-300">Giáo viên đã mở khoá chọn máy cho buổi {session?.title??""} · {session?.session_date?new Date(`${session.session_date}T00:00:00`).toLocaleDateString("vi-VN"):""}</p><MachineDutyPanel sessionId={row.session_id} machineCode={row.machine_code} checkedInAt={row.checked_in_at??undefined} sessionClosed={session?.status==="closed"} override={true} onMachineChosen={load}/></div>})}
...
const MACHINE_EDIT_WINDOW_MS=6*60*60*1000;

function MachineDutyPanel({sessionId,machineCode,checkedInAt,sessionClosed,override,onMachineChosen}:{sessionId:number;machineCode:string|null;checkedInAt?:string;sessionClosed?:boolean;override?:boolean;onMachineChosen:()=>void}){
  ...
  const withinEditWindow=override||(!sessionClosed&&(!checkedInAt||now-new Date(checkedInAt).getTime()<=MACHINE_EDIT_WINDOW_MS));

  async function chooseMachine(){if(!picked||!withinEditWindow)return;setBusy(true);setError("");try{await selectAttendanceMachine(sessionId,picked);setEditing(false);onMachineChosen()}catch(cause){setError(cause instanceof Error?cause.message:"Không chọn được máy.")}finally{setBusy(false)}}
  ...
  if(!machineCode||editing)return <div className="mt-3 rounded-xl border border-orange-400/20 bg-orange-500/5 p-3">
    ...
    {withinEditWindow?<div className="mt-2 flex flex-wrap gap-2">
    ...
    </div>:<p className="mt-2 text-xs text-slate-500">{sessionClosed?"Buổi điểm danh đã bị khoá, không thể chọn/đổi máy.":"Đã hết thời gian chọn/đổi máy (chỉ trong 6 tiếng kể từ lúc bạn điểm danh)."}</p>}
    {error&&<p className="mt-2 text-xs text-red-300">{error}</p>}
  </div>;

  return <div className="mt-3 space-y-2">
    <div className="rounded-xl border border-orange-400/20 bg-orange-500/5 p-3">
      <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-bold text-white"><Wrench size={15} className="text-orange-300"/>Vệ sinh máy {machineCode}</div>{withinEditWindow&&<button type="button" onClick={()=>setEditing(true)} className="flex items-center gap-1 text-xs font-bold text-orange-300 hover:text-orange-200"><Pencil size={12}/>Đổi máy</button>}</div>
