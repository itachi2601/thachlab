...
function MachineDutyPanel({sessionId,machineCode,checkedInAt,sessionClosed,onMachineChosen}:{sessionId:number;machineCode:string|null;checkedInAt?:string;sessionClosed?:boolean;onMachineChosen:()=>void}){
  ...
  const withinEditWindow=!sessionClosed&&(!checkedInAt||now-new Date(checkedInAt).getTime()<=MACHINE_EDIT_WINDOW_MS);
