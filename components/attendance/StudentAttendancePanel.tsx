...
import { CalendarCheck, ChevronDown, ChevronUp, CheckCircle2, Clock3, Wrench, Camera, Check, Loader2, Pencil, Dices } from "lucide-react";
...
function MachineDutyPanel({sessionId,machineCode,open,onMachineChosen}:{sessionId:number;machineCode:string|null;open:boolean;onMachineChosen:()=>void}){
  ...
  if(!machineCode||editing)return <div className="mt-3 rounded-xl border border-orange-400/20 bg-orange-500/5 p-3">
    ...
    {machinesError?<p role="alert" className="mt-2 text-xs text-red-300">{machinesError}</p>:!machines.length?<p className="mt-2 text-xs text-slate-500">Giáo viên chưa chọn máy cho buổi học này.</p>:open?<div className="mt-2 flex flex-wrap gap-2">
      <select value={picked} onChange={e=>setPicked(e.target.value)} className="min-w-40 flex-1 rounded-lg border border-white/10 bg-panel-deep px-3 py-2 text-sm text-white">
...
function MachineCheckpointSlot({sessionId,machineCode,checkpoint,label,icon:Icon=Camera,photos,onUploaded,canUpload=true,duty=false}:{sessionId:number;machineCode:string;checkpoint:MachineCheckpoint;label:string;icon?:typeof Camera;photos:AttendanceMachinePhoto[];onUploaded:()=>void;canUpload?:boolean;duty?:boolean}){
  ...
  return <div className="rounded-lg border border-white/10 bg-panel-deep p-2.5">
