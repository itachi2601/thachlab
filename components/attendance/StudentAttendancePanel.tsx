...
import { CalendarCheck, ChevronDown, ChevronUp, CheckCircle2, CheckCheck, Clock3, Copy, ShieldCheck, Wrench, Camera, Check, Loader2, Pencil, Dices } from "lucide-react";
import { createAttendanceSession, fetchMyAttendance, submitAttendanceCode, type AttendanceSession } from "@/services/course-attendance";
import { fetchMyClassRole, type ClassRole } from "@/services/course-enrollments";
import { fetchSessionMachines, groupMachinesByType, selectAttendanceMachine, isPhotoDutyMachine, fetchAttendanceMachinePhotos, submitAttendanceMachinePhoto, createAttendanceMachinePhotoUrl, type AttendanceMachinePhoto, type MachineCheckpoint, type Machine } from "@/services/attendance-machine";

const labels:Record<string,string>={present:"Có mặt",late:"Đi muộn",excused:"Vắng có phép",absent:"Vắng không phép"};
const classOfficerLabels:Record<string,string>={lop_truong:"Lớp trưởng",lop_pho_1:"Lớp phó 1",lop_pho_2:"Lớp phó 2"};
...
function ClassOfficerAttendanceOpener({courseId,classRole}:{courseId:number;classRole:ClassRole}){
  const[opened,setOpened]=useState<AttendanceSession|null>(null);
  const[busy,setBusy]=useState(false);
  const[error,setError]=useState("");
  const[copied,setCopied]=useState(false);
  async function openForClass(){
    setBusy(true);setError("");
    try{const now=new Date();const item=await createAttendanceSession({courseId,title:"Điểm danh lớp",startsAt:now.toISOString(),durationMinutes:60,lateMinutes:15});setOpened(item)}
    catch(cause){setError(cause instanceof Error?cause.message:"Không mở được điểm danh.")}
    finally{setBusy(false)}
  }
  function copyCode(){if(!opened)return;navigator.clipboard.writeText(opened.code);setCopied(true);setTimeout(()=>setCopied(false),1500)}
  return <div className="mb-3 rounded-xl border border-orange-400/20 bg-orange-500/5 p-3">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="flex items-center gap-2 text-sm font-bold text-white"><ShieldCheck size={16} className="text-orange-300"/>{classOfficerLabels[classRole]} · có thể mở điểm danh cho cả lớp</div>
      <button disabled={busy} onClick={openForClass} className="shrink-0 rounded-lg bg-orange-600 px-3 py-1.5 text-xs font-bold text-white disabled:opacity-40">{busy?"Đang mở…":opened?"Mở buổi mới":"Mở điểm danh cho lớp"}</button>
    </div>
    {opened&&<div className="mt-3 flex items-center gap-3 rounded-lg bg-black/20 p-2.5"><span className="text-xs text-slate-400">Mã điểm danh, đọc cho cả lớp:</span><strong className="font-mono text-2xl tracking-[.18em] text-white">{opened.code}</strong><button onClick={copyCode} className="text-orange-300">{copied?<CheckCheck size={16}/>:<Copy size={16}/>}</button></div>}
    {error&&<p className="mt-2 text-xs text-red-300">{error}</p>}
  </div>;
}

export default function StudentAttendancePanel({courseId,studentId}:{courseId:number;studentId:string}){const[code,setCode]=useState("");const[rows,setRows]=useState<Awaited<ReturnType<typeof fetchMyAttendance>>>([]);const[message,setMessage]=useState("");const[busy,setBusy]=useState(false);const[expanded,setExpanded]=useState(false);const[classRole,setClassRole]=useState<ClassRole>("member");const load=useCallback(()=>fetchMyAttendance(courseId,studentId).then(setRows).catch(()=>undefined),[courseId,studentId]);useEffect(()=>{void load()},[load]);useEffect(()=>{void fetchMyClassRole(courseId,studentId).then(setClassRole).catch(()=>undefined)},[courseId,studentId]);const attended=rows.filter(row=>row.status==="present"||row.status==="late").length;const rate=rows.length?Math.round(attended/rows.length*100):0;const visibleRows=expanded?rows:rows.slice(0,4);const latest=rows[0];const otherOpenRows=rows.filter(row=>row!==latest&&(row.status==="present"||row.status==="late")&&sessionOf(row)?.machine_selection_open);async function submit(){setBusy(true);try{const result=await submitAttendanceCode(courseId,code);setMessage(`${result.status==="late"?"Điểm danh muộn":"Điểm danh thành công"}: ${result.session_title}`);setCode("");await load()}catch(cause){setMessage(cause instanceof Error?cause.message:"Không thể điểm danh.")}finally{setBusy(false)}}return <section className="mt-5 rounded-2xl border border-cyan-400/15 bg-cyan-500/5 p-4">{classRole!=="member"&&<ClassOfficerAttendanceOpener courseId={courseId} classRole={classRole}/>}<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><CalendarCheck size={18} className="text-cyan-300"/><strong className="text-white">Điểm danh offline</strong></div><p className="mt-1 text-xs text-slate-400">Đã tham dự {attended}/{rows.length} buổi · {rate}%</p></div><div className="flex gap-2"><input inputMode="numeric" maxLength={6} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,""))} placeholder="Mã 6 số" className="w-full min-w-0 flex-1 rounded-xl border border-white/10 bg-[#080d1d] px-3 py-2 font-mono text-sm tracking-wider text-white sm:w-32 sm:flex-none"/><button disabled={busy||code.length!==6} onClick={submit} className="shrink-0 rounded-xl bg-cyan-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-40">Xác nhận</button></div></div>{message&&<p className="mt-3 rounded-lg bg-black/20 p-2 text-xs text-cyan-100">{message}</p>}
...
function MachineDutyPanel({sessionId,machineCode,open,onMachineChosen}:{sessionId:number;machineCode:string|null;open:boolean;onMachineChosen:()=>void}){
  ...
  return <div className="mt-3 rounded-xl border border-orange-400/20 bg-orange-500/5 p-3">
